...
! private variables
 ...
 &(/'Fatal  ','Serious','Warning', 'Info   ', 'Debug  ','Info   '/)
  ...
  type(error_buffer_t) :: error_buffer !< local variable
...
subroutine log_init(fileroot,timestamp,decomp)
  ...
  if(present(decomp)) then
     ! If the decomp structure is included then this is a call from powcal
...
subroutine log_error(modname,subname,point,severity,errormessage,writeout)
  ...
  logical, intent(in), optional :: writeout !< local variable

  !! local
  !> errors default to write to screen, change to false to write
  !! to a buffer that can be periodically flushed
  logical :: flush = .true. !< .
  ...
  if(.NOT.flush) then
     ...
     ! This errorno is only updated during a flush, errorno is also stored independently in the error_buffer
     errorno=errorno+1
...
subroutine grow_error_buffer
  ...
  type(error_line_t),dimension(:), allocatable :: new_error_line !< local variable
  integer(ki4) :: newsize !< local variable

  newsize = 2 * error_buffer%size

  ! First check if the buffer is too big and flush and return if it is
  if(newsize > ebufmaxsize) then
     call log_flusherror()
     return
  endif
...
subroutine log_flusherror
  ...
  do i=1,error_buffer%filled
     call log_error(error_buffer%error_line(i)%modname, &
     error_buffer%error_line(i)%subname,error_buffer%error_line(i)%point, &
