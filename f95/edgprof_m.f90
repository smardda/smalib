...
subroutine edgprof_readcon(self,kin)
  ...
  character(len=80) :: type_of_positions !< whether in distance or flux from SOL
 ...
 &diffusion_length, &
 &decay_length_near, &
 &positions, deposition_profile, type_of_positions, number_of_positions,&
 ...
 &general_integer_parameters, number_of_integer_parameters
  ...
  type_of_positions='radius'
     ...
     if(status/=0) then
        call log_error(m_name,s_name,1,error_fatal,'Error reading edgprof parameters')
        print '("Fatal error reading edgprof parameters")'
     end if
 ...
 &      call log_error(m_name,s_name,34,error_fatal,'power_loss (W) must be >0')
     ...
     case('samples')
        if(type_of_positions/='radius'.AND.type_of_positions/='flux') &
 ...
 &      call log_error(m_name,s_name,48,error_fatal,'no parameters set')
     ...
     self%postype=type_of_positions
